...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download time series of Sentinel 1 images"
    )
    ...
    requests: List[IngestionRequest] = [Sentinel1AscRequest(), Sentinel1DescRequest()]
    names = ["asc", "des"]
    for name, request in zip(names, requests):
        print("Getting data:", type(request))

        for series_id in tqdm(ids, desc="Series", position=0):
            curr_tile = tiles.loc[series_id]

            out_dir = os.path.join(args.out, str(series_id), name)
